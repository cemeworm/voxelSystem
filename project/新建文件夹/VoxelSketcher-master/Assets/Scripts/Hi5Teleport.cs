...
{
	//-------------------------------------------------------------------------
	public class Hi5Teleport : MonoBehaviour
	{
		...
		public Hi5_Glove_Interaction_Hand pointerHand;
		...
		public Transform World_Menu_Button;
...
/*		[Header("Audio Sources")]
		...
		//-------------------------------------------------
		private static Hi5Teleport _instance;
		public static Hi5Teleport instance
		{
			get
			{
				if (_instance == null)
				{
					_instance = GameObject.FindObjectOfType<Hi5Teleport>();
				}
		...
		void Start()
		{
			...
			HMD = GameObject.Find("HMDPosition").GetComponent<Transform>();
			World_Menu_Button = GameObject.Find("World_Menu_Button").GetComponent<Transform>();
...
/*			if (player == null)
		...
		void Update()
		{
			Hi5_Glove_Interaction_Hand newPointerHand = null;
			...
			/*			foreach (Hand hand in player.hands)
						{
							...
							{
								HI5_Left_Human_Collider = hand;
							}
						}*/
			if (vrcon.teleportInput() == 3)
			{
				TryTeleportPlayer();

			}

			if (vrcon.teleportInput() == 2)
			{
				newPointerHand = HI5_Left_Human_Collider;
			...
			else
			{
				...
				{
					if (vrcon.teleportInput() == 0 && newPointerHand == null)
					{
						//Hide the pointer
						HidePointer();
...
/*			if (oldPointerHand)
			...
			pointerHand = HI5_Left_Human_Collider;
...
/*			if (visible)
			...
			if (pointerHand)
			{
				...
				/*				if (HI5_Left_Human_Collider.currentAttachedObject != null)
								{
									allowTeleportWhileAttached = HI5_Right_Human_Collider.currentAttachedObject.GetComponent<AllowTeleportWhileAttachedToHand>();
								}*/

				//Keep track of any existing hovering interactable on the hand
				//originalHoverLockState = HI5_Left_Human_Collider.hoverLocked;
				//originalHoveringInteractable = HI5_Left_Human_Collider.hoveringInteractable;

				/*				if (ShouldOverrideHoverLock())
								{
									HI5_Right_Human_Collider.HoverLock(null);
								}*/
